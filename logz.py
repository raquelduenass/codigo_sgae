...
def configure_output_dir(d=None):
    ...
    # assert not osp.exists(G.output_dir), "Log dir %s already exists!
    # Delete it first or use a different dir"%G.output_dir
...
def log_tabular(key, val):
    ...
    if G.first_row:
        ...
        assert key in G.log_headers, "Trying to introduce a new key %s that you didn't" \
                                     "include in the first iteration" % key
    assert key not in G.log_current_row, "You already set %s this iteration." \
